...
在编码阶段通过 CLI 进行交互式安全审查，实时辅助开发人员发现并修复安全问题。
...
进入安全审查后，系统会询问您想要审查的内容，支持以下三种审查范围：

| 范围 | 说明 |
|------|------|
| 指定文件 | 审查指定的单个文件，适用于针对特定文件的安全排查 |
| 指定目录 | 审查指定目录及其子目录下的所有代码文件，适用于审查指定模块或组件 |
| 指定分支 | 审查指定 Git 分支的代码变更，适用于审查待合并的分支代码 |

![指定审查范围](./img/CLI使用2-指定审查范围.png)

### 步骤 3：查看审查报告

触发安全审查后，CLI 交互窗口会实时展示审查过程。审查过程中如涉及危险操作，需要用户手动确认后方可继续。审查时长与代码量有关，从几分钟到几十分钟不等。审查完成后，会在项目本地生成安全审查报告。报告包含以下三种类型：
